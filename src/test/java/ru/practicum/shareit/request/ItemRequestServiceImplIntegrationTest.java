...
class ItemRequestServiceImplIntegrationTest {
    ...
    void getAllRequests_pagination() {
        User current = userRepository.save(new User(null, "Георгий1", "current@email.com"));
        User other = userRepository.save(new User(null, "Георгий2", "other@email.com"));

        for (int i = 1; i <= 5; i++) {
            createRequest(other, "Запрос " + i);
        }

        var page1 = itemRequestService.getAllRequests(current.getId(), 0, 2);
        assertThat(page1, hasSize(2));
        assertThat(page1.get(0).getDescription(), equalTo("Запрос 5"));
        assertThat(page1.get(1).getDescription(), equalTo("Запрос 4"));

        var page2 = itemRequestService.getAllRequests(current.getId(), 2, 2);
        assertThat(page2, hasSize(2));
        assertThat(page2.get(0).getDescription(), equalTo("Запрос 3"));
        assertThat(page2.get(1).getDescription(), equalTo("Запрос 2"));

        var page3 = itemRequestService.getAllRequests(current.getId(), 4, 2);
        assertThat(page3, hasSize(1));
        assertThat(page3.get(0).getDescription(), equalTo("Запрос 1"));
    }
